// Run this in the browser console while logged into /admin.html
// Make sure you're authenticated as an admin first!
...
// Import Firebase (already available on admin page)
import('https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js').then(async ({ collection, getDocs, deleteDoc, doc, addDoc, getFirestore }) => {
  ...
  for (let i = 0; i < newCubes.length; i++) {
    ...
    console.log(`  Added: ${cube.title}`);
  }
  console.log(`Done! Added ${newCubes.length} cubes. Refresh the page.`);
});
